...
// -> select a larger 'Partition Scheme'  e.g. using 'No OTA (2MB APP / 2MB SPIFFS) ***
...
#include <SPI.h>   // added: SPI is used for SD card
...
#define pin_VOL_POTI 1
#define pin_repeat 37
...
#define PIN_SD_AMP  6 // optional: pin to enable amplifier (e.g. MAX98357) via GPIO 6 (or any other GPIO pin)

// SD Card pins (ESP32-S3 wiring)
...
void setup() {
  ...
  digitalWrite(PIN_SD_AMP, HIGH); // enable amplifier
...
here:
  ...
  if (Timer.isReady()) {
    battery_filtering(); // fixed typo: was 'battry_filtering()'
...
void led_RGB(int red, int green, int blue) {
  ...
  // Common anode: LED is ON when pin is LOW
  red = 255 - red;
  green = 255 - green;
  blue = 255 - blue;
